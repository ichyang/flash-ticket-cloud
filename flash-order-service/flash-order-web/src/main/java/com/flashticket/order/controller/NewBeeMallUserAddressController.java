 ...
 * 版权所有，侵权必究！
 */
package com.flashticket.order.controller;
...
import com.flashticket.order.config.annotation.TokenToMallUser;
import com.flashticket.order.controller.param.SaveMallUserAddressParam;
import com.flashticket.order.controller.param.UpdateMallUserAddressParam;
import com.flashticket.order.controller.vo.FlashTicketUserAddressVO;
import com.flashticket.order.entity.MallUserAddress;
import com.flashticket.order.service.FlashTicketUserAddressService;
...
@RequestMapping("/mall")
public class FlashTicketUserAddressController {

    @Resource
    private FlashTicketUserAddressService mallUserAddressService;

    @GetMapping("/address")
    @ApiOperation(value = "我的收货地址列表", notes = "")
    public Result<List<FlashTicketUserAddressVO>> addressList(@TokenToMallUser MallUserToken loginMallUserToken) {
    ...
    public Result<FlashTicketUserAddressVO> getMallUserAddress(@PathVariable("addressId") Long addressId,
                                                              @TokenToMallUser MallUserToken loginMallUserToken) {
        MallUserAddress mallUserAddressById = mallUserAddressService.getMallUserAddressById(addressId);
        FlashTicketUserAddressVO newBeeMallUserAddressVO = new FlashTicketUserAddressVO();
