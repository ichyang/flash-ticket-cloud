...
import com.flashticket.ticket.config.annotation.TokenToAdminUser;
import com.flashticket.ticket.controller.param.BatchIdParam;
import com.flashticket.ticket.controller.param.GoodsAddParam;
import com.flashticket.ticket.controller.param.GoodsEditParam;
import com.flashticket.ticket.entity.GoodsCategory;
import com.flashticket.ticket.entity.LoginAdminUser;
import com.flashticket.ticket.entity.FlashTicketGoods;
import com.flashticket.ticket.entity.UpdateStockNumDTO;
import com.flashticket.ticket.service.FlashTicketCategoryService;
...
public class FlashTicketAdminTicketController {

    private static final Logger logger = LoggerFactory.getLogger(FlashTicketAdminTicketController.class);

    @Resource
    private FlashTicketTicketService newBeeMallGoodsService;
    @Resource
    private FlashTicketCategoryService newBeeMallCategoryService;
    ...
    public Result list(@RequestParam(required = false) @ApiParam(value = "页码") Integer pageNumber,
        ...
        return ResultGenerator.genSuccessResult(newBeeMallGoodsService.getFlashTicketGoodsPage(pageUtil));
    }
    ...
    public Result save(@RequestBody @Valid GoodsAddParam goodsAddParam, @TokenToAdminUser LoginAdminUser adminUser) {
        logger.info("adminUser:{}", adminUser.toString());
        FlashTicketGoods newBeeMallGoods = new FlashTicketGoods();
        BeanUtil.copyProperties(goodsAddParam, newBeeMallGoods);
        String result = newBeeMallGoodsService.saveFlashTicketGoods(newBeeMallGoods);
    ...
    public Result update(@RequestBody @Valid GoodsEditParam goodsEditParam, @TokenToAdminUser LoginAdminUser adminUser) {
        ...
        String result = newBeeMallGoodsService.updateFlashTicketGoods(newBeeMallGoods);
        if (ServiceResultEnum.SUCCESS.getResult().equals(result)) {
            return ResultGenerator.genSuccessResult();
        } else {
            return ResultGenerator.genFailResult(result);
    ...
    public Result info(@PathVariable("id") Long id, @TokenToAdminUser LoginAdminUser adminUser) {
        ...
        FlashTicketGoods goods = newBeeMallGoodsService.getFlashTicketGoodsById(id);
        if (goods == null) {
    ...
    public Result getFlashTicketGoodsByIds(@RequestParam("goodsIds") List<Long> goodsIds) {
        List<FlashTicketGoods> newBeeMallGoods = newBeeMallGoodsService.getFlashTicketGoodsByIds(goodsIds);
