...
public class TokenToMallUserMethodArgumentResolver implements HandlerMethodArgumentResolver {
    ...
    @Autowired
    private FlashTicketUserMapper mallUserMapper;
        ...
        if (parameter.getParameterAnnotation(TokenToMallUser.class) instanceof TokenToMallUser) {
            ...
            if (null != token && !"".equals(token) && token.length() == 32) {
                ...
                if (mallUserToken == null ) {
                    FlashTicketException.fail(ServiceResultEnum.TOKEN_EXPIRE_ERROR.getResult());
                }
                MallUser mallUser = mallUserMapper.selectByPrimaryKey(mallUserToken.getUserId());
                if (mallUser == null) {
                    FlashTicketException.fail(ServiceResultEnum.USER_NULL_ERROR.getResult());
                }
                if (mallUser.getLockedFlag().intValue() == 1) {
                    FlashTicketException.fail(ServiceResultEnum.LOGIN_USER_LOCKED_ERROR.getResult());
                }
                return mallUserToken;
            } else {
                FlashTicketException.fail(ServiceResultEnum.NOT_LOGIN_ERROR.getResult());
            }
        }
        return null;
    }
